...
def test_errors(train_list, test_list, best_parameters):
    ...
    for i in range(len(train_list)):
        ...
        rmse_fair = mean_absolute_percentage_error(y_te, predictions)
...
spatial_cv = list_reader("spatial_cv")
# %% define parameters
...
train = pd.read_csv("./FairTrainTest/Files/Datasets/Ignore/available_data.csv")
...
# %% tuned fair model
best_params = {
    "n_estimators": 499,  # 729
    "min_samples_leaf": 9,  # 9
    "max_leaf_nodes": 10,  # 29
    "max_depth": 16,  # 11
    "random_state": 42  # 42
}
...
rmse_fair_rw = mean_absolute_percentage_error(y_test_, predict)
...
model.fit(X_train_, y_train_)
predict = model.predict(X_test_)
rmse_vsa_rw = mean_absolute_percentage_error(y_test_, predict)

# %% print results
print("Mean RMSE 100 test sets: ")
print(f"Fair approach: {np.mean(rmse_fair_test):.2%} \u00B1 {np.std(rmse_fair_test):.2%}")
print(f"VSA approach : {np.mean(rmse_vsa_test):.2%} \u00B1 {np.std(rmse_vsa_test):.2%}\n")
print("RMSE in real world")
print(f"Fair approach: {rmse_fair_rw:.2%}")
print(f"VSA approach : {rmse_vsa_rw:.2%}\n")
